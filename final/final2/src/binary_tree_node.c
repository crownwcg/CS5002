...
void initBinaryTreeNode(BinaryTreeNode* node, BinaryTreeNodeData* data) {
	node->data = data;
	node->linkTo[leftLink] = NULL;
	node->linkTo[rightLink] = NULL;
	node->linkTo[parentLink] = NULL;
}
 ...
 * @return a new BinaryTreeNode
 */
BinaryTreeNode* createBinaryTreeNode(BinaryTreeNodeData* data) {
...
int binaryTreeNodeCardinality(BinaryTreeNode* node) {
	return    (node->linkTo[leftLink] != NULL)
			+ (node->linkTo[rightLink] != NULL);
}
...
bool isBinaryTreeNodeRoot(BinaryTreeNode* node) {
	return (node->linkTo[parentLink] == NULL);
}
