...
/**
 * Create an array deque with an initial capacity and max capacity.
 *
 * @param initialCapacity initial capacity of the ArrayDeque.
 ...
 */
ArrayDeque *createArrayDeque(size_t initialCapacity, size_t maxCapacity) {
	ArrayDeque *deque = malloc(sizeof(ArrayDeque));
	deque->list = createArrayList(initialCapacity, maxCapacity);
 ...
 */
bool deleteFirstArrayDequeVal(ArrayDeque *deque, const char **val) {
	const char* localVal;
 ...
 * @return if deque is empty
 */
bool deleteLastArrayDequeVal(ArrayDeque *deque, const char **val) {
	const char* localVal;
	// copy first because drop causes returned value to be freed
 ...
 * @return if deque is empty
 */
bool dequeueArrayDequeVal(ArrayDeque *deque, const char **val) {
	const char* localVal;
	// copy first because drop causes returned value to be freed
 ...
 * @return if deque is empty
 */
bool popArrayDequeVal(ArrayDeque *deque, const char **val) {
	const char* localVal;
	// copy first because drop causes returned value to be freed
...
void printStackArrayDeque(ArrayDeque *deque) {
	...
	for (int i = size; i > 0; i--) {
		getArrayListValAt(deque->list, i-1, &val);
		printf("%s\n", val);
	}
}
